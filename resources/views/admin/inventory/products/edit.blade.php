...
<div class="max-w-4xl mx-auto">
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <form action="{{ route('admin.products.update', $product) }}" method="POST" enctype="multipart/form-data" class="p-8 space-y-6">
            ...
            <div class="bg-indigo-50/50 p-8 rounded-[2.5rem] border border-indigo-100/50 space-y-6" x-data="{ alertType: '{{ $product->low_stock_threshold ? 'individual' : 'global' }}' }">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div class="space-y-4">
                        ...
                        <div class="flex flex-col gap-3">
                            <label class="flex items-center gap-3 p-4 bg-white rounded-2xl border border-indigo-100 cursor-pointer hover:bg-indigo-50 transition-all group">
                                <input type="radio" name="threshold_type" value="global" x-model="alertType" class="w-5 h-5 text-indigo-600 focus:ring-indigo-500/20">
                    ...
                    <div class="space-y-4" x-show="alertType === 'individual'" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-x-4" x-transition:enter-end="opacity-100 translate-x-0">
                        ...
                        <input type="number" name="low_stock_threshold" value="{{ old('low_stock_threshold', $product->low_stock_threshold) }}" min="1" class="w-full bg-white border-indigo-200 rounded-2xl p-5 text-sm font-black text-indigo-950 focus:ring-4 focus:ring-indigo-500/10 outline-none shadow-inner" placeholder="e.g. 50">
